...
Its static-musl text and GUI build supports IPv4 HTTP and HTTPS. The pinned
OpenSSL build, DNS fork helper, pipe readiness, and `select(2)` event loop have
restricted-QEMU coverage, including valid DNS/IP HTTPS, SNI, TLS 1.2,
redirects, and strict rejection of mismatched, untrusted, expired, and
future-dated certificates. BusyBox `wget` remains HTTP-only.
