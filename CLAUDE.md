...
`TERMINAL.ELF` is the only interactive terminal. It owns the VT parser,
screen grid, scrollback, key encoding, font rasterization, and GUI surface in
ring 3; the kernel retains only the PTY, termios, winsize, line discipline, and
fd readiness. Both the Start menu and `/bin/terminal` execute the ELF directly.
Its current v1 window is fixed at 80×24 with an opaque `#202020` surface; the
former translucent kernel-terminal surface was removed with the kernel
emulator.

Resizable frames expose functional minimize/maximize/restore controls:
minimize preserves the taskbar button and normal/maximized placement, while
maximize fills the work area above the taskbar; fixed dialogs remain
close-only.
...
Kernel-requested programs use one persistent `process-service`; it launches the
ring-3 desktop shell and later reaps detached exits from its own stack. Once
running, `DESKTOP.ELF` launches Start, Run, and Terminal children with ordinary
ring-3 `fork`/`execve`.
...
- `src/terminal/` — Kernel PTY service: master/slave queues, termios, winsize,
  canonical line discipline, ISIG, and fd readiness. VT emulation lives in
  `userland/libs/vte` and `TERMINAL.ELF`. See
  [`src/terminal/CLAUDE.md`](src/terminal/CLAUDE.md).
...
5. **Network scope is deliberately small** — One polling modern VirtIO NIC with IPv4 and DHCP-backed DNS; Links2 alone has HTTPS, while IPv6 and NIC interrupts are follow-ups.
