...
This first browser port is deliberately text-mode IPv4/HTTP-only. Its static
musl build, DNS fork helper, pipe readiness, and `select(2)` event loop have
restricted-QEMU numeric and hostname HTTP coverage. The cryptographic entropy
prerequisite has landed, but HTTPS still needs a separate TLS trust-stack
integration; do not enable it merely as a build flag.
...
- `./build.sh --rebuild-userland` — Recompile prebuilt-managed userland apps (zsh, BusyBox, TinyCC, Links, future Linux ports) from source instead of copying the committed `userland/prebuilt/<NAME>.ELF`. Same flag works on `test.sh`. Env equivalents: `REBUILD_USERLAND=1`, or per-app `REBUILD_ZSH=1` / `REBUILD_TCC=1` / `REBUILD_LINKS2=1`. See `userland/prebuilt/README.md`.
- `./build.sh -h` — Show help and usage
- `cargo build` — Build the kernel only (won't create disk images)
- `cargo build --release` — Build optimized release version

**Prebuilt userland ELFs**: fetched/slow upstream ports including `ZSH.ELF`, `BB.ELF`, `TCC.ELF`, and `LINKS.ELF` ship as committed binaries under `userland/prebuilt/`. Fresh clones boot them without the `x86_64-linux-musl-cross` toolchain installed. `HELLO.ELF` (Rust) and `HELLOCPP.ELF` (small C++ wrapper) are NOT prebuilt — they build from source on every run. After changing the upstream source / Makefile / patches of a prebuilt-managed app, run `./userland/refresh-prebuilt.sh` and commit the updated binary alongside the source change.
