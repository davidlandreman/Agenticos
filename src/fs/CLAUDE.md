...
- `p9/` — 9P2000.L client backend for the virtio-9p host share at `/shared` (`protocol.rs` wire codec, `client.rs` tagged fid/ops client, `filesystem.rs` trait impl). Four independently attached/tagged lanes let Git's index workers overlap requests. Each lane uses a scheduler-aware FIFO lock because its guard spans interrupt-driven RPCs; contenders must park, never spin behind a sleeping owner. Reusable path fids remove repeated walk/clunk traffic, while every metadata lookup still sends a fresh Tgetattr for concurrent multi-instance coherence. Open reads use one lane-local negotiated-size read-ahead window, discarded on write/close, to coalesce Git/libc's small sequential reads.
...
- VirtIO-9p RPCs also sleep until PCI completion. Per-lane serialization must remain scheduler-aware; replacing `P9ClientLock` with a spin mutex deadlocks concurrent Git/pthread callers on `/shared`.
- Ring-3 `getdents64` opens directories through `Directory::open_names`, allowing 9p `Rreaddir` names/types to pass through without an eager walk/getattr/clunk triplet for every entry. Full `Directory::open` / `enumerate_dir` still fetches sizes and timestamps for callers that consume them.
