    ...
    unsafe {
        ...
        if let Some(response) = read_data_timeout() {
            if response == ACK {
                ...
                debug_info!(
                    "Mouse command 0x{:02x} got response 0x{:02x} instead of ACK",
                    cmd,
                    response
                );
            }
...
pub fn handle_interrupt(data: u8) {
    ...
    debug_trace!(
        "Mouse packet byte {} of 3: 0x{:02x}",
        mouse_data.packet_index,
        data
    );

    if mouse_data.packet_index >= 3 {
        debug_trace!(
            "Processing complete mouse packet: 0x{:02x} 0x{:02x} 0x{:02x}",
...
fn process_packet(data: &mut MouseData) {
    ...
    if data.buttons != old_buttons {
        let left = if data.buttons & 0x01 != 0 {
            "pressed"
        } else {
            "released"
        };
        let right = if data.buttons & 0x02 != 0 {
            "pressed"
        } else {
            "released"
        };
        let middle = if data.buttons & 0x04 != 0 {
            "pressed"
        } else {
            "released"
        };
        debug_info!(
            "Mouse buttons changed: left={}, right={}, middle={}",
            left,
            right,
            middle
        );
