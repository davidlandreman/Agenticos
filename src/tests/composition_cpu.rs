...
fn test_backdrop_blur_uniform_is_identity() {
    ...
    let mut surfaces = BTreeMap::new();
    surfaces.insert(backdrop_id, solid_surface(7, 3, PremulArgb::from_rgba(30, 90, 150, 255)));
    surfaces.insert(glass_id, solid_surface(7, 3, PremulArgb::from_rgba(220, 235, 250, 160)));
...
fn test_backdrop_blur_spreads_impulse() {
    ...
    surfaces.insert(glass_id, solid_surface(9, 1, PremulArgb::from_rgba(0, 0, 0, 1)));
    ...
    engine.compose(&scene, &surfaces, &[Rect::new(0, 0, 9, 1)]).unwrap();
...
fn test_backdrop_damage_uses_current_neighborhood() {
    ...
    let glass_id = SurfaceId(2);
    let mut surfaces = BTreeMap::new();
    surfaces.insert(backdrop_id, solid_surface(7, 1, PremulArgb::from_rgba(0, 0, 255, 255)));
    surfaces.insert(glass_id, solid_surface(1, 1, PremulArgb::from_rgba(255, 255, 255, 128)));
    ...
    engine.compose(&scene, &surfaces, &[Rect::new(0, 0, 7, 1)]).unwrap();
    let before = engine.output().pixel(3, 0).unwrap();
    surfaces.get_mut(&backdrop_id).unwrap()
        .set_pixel(1, 0, PremulArgb::from_rgba(255, 0, 0, 255));
    engine.compose(&scene, &surfaces, &[Rect::new(1, 0, 3, 1)]).unwrap();
