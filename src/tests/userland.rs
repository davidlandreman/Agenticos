...
/// The shipped zsh startup chain must survive all three integration layers:
/// host-share staging, the `/etc` rewrite, and FAT long-name subdirectories.
fn test_dispatch_stat_zsh_config_via_rewriter() {
    assert!(
        crate::fs::exists("/host/etc/zshrc"),
        "test.sh must stage /host/etc/zshrc"
    );
    assert!(
        crate::fs::exists("/host/etc/zsh/functions/promptinit"),
        "test.sh must stage the pruned zsh function library"
    );
...
pub fn get_tests() -> &'static [&'static dyn Testable] {
    &[
        ...
        &test_dispatch_stat_zsh_config_via_rewriter,
