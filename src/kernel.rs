...
pub fn init(boot_info: &'static mut BootInfo) {
    ...
    // Mouse, the desktop root/shell, and host bridges are not exercised by any
    // in-kernel test. Skipping them under `feature = "test"` removes the
    // largest chunks of `./test.sh` startup latency: the desktop root paints
    // the wallpaper + initial render; the MCP bridge spawns a kernel process,
    ...
    #[cfg(not(feature = "test"))]
    {
        ...
        debug_info!("[boot] desktop root (ring-3 shell)");
        crate::commands::desktop::init_desktop_root_only();
        window::render_frame();
    ...
    {
        let _ = screen_dims;
        debug_info!("[boot] test mode — skipping mouse, desktop shell, host bridges");
    }
...
pub fn run() -> ! {
    ...
    // detached-process teardown. Start it before the desktop shell can submit
    // launch requests.
    debug_info!("Spawning user process service...");
    crate::userland::process_service::start();

    // Launch the ring-3 desktop shell (DESKTOP.ELF). It claims the shell role
    // via gui_shell_register and drives the taskbar/Start menu through the
    // desktop-shell protocol syscalls.
    debug_info!("Launching ring-3 desktop shell (DESKTOP.ELF)...");
    crate::commands::desktop::spawn_ring3_desktop_shell();
